...
// =============================================================================
// de10_lite_avl_mm_sdram_demo_tb.sv
//
// Board-level simulation of the SDRAM demonstration, against Intel's generated
// functional SDRAM model.
//
// WHAT THIS DOES AND DOES NOT PROVE
// ---------------------------------
// The model is functional, not timing-accurate: it decodes LOAD MODE REGISTER,
// ACTIVATE, READ and WRITE and pipelines read data by the CAS latency, but it
// does not model tRCD, tRP, tRFC, tWR, the refresh interval, or data
// retention. So this testbench proves that the controller drives the right
// commands to the right addresses and returns the right data, and that the
// sequencer and its checker behave. It does NOT prove the timing parameters
// are right - only hardware does that, and run_on_board.sh is what does it.
//
...
//
// Run with: simulation/questa/run_sim.tcl
// =============================================================================

module de10_lite_avl_mm_sdram_demo_tb;
    ...
    // Intel's generated functional model - see simulation/gen_mem_model.sh.
    sdram_mem_model u_mem (
    ...
    initial begin
        ...
        $display("\n=============================================================");
        $display(" SDRAM demo vs Intel's generated functional model");
        $display("=============================================================");
        ...
        fork
            ...
            begin
                ...
                u_mem.mem_array[25'd700] = ~patt(25'd700);
            end
        join
        ...
        if (checks_failed == 0) $display("  *** ALL TESTS PASSED ***");

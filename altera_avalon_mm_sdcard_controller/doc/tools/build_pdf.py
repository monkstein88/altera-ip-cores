...
def captions(html_text):
    ...
    # ...and the other order. A caption belongs BELOW its figure, which is how
    # both of this core's documents are written; matching only caption-first
    # left every figure unwrapped, so none of them got the landscape treatment
    # and each one silently overflowed the portrait column.
    def fig2(m):
    ...
    html_text = re.sub(
        r"<p>(<img\b[^>]*/?>)</p>\s*<p><strong>(Figure \d+\..*?)</strong></p>",
        fig2, html_text, flags=re.S)

    # An image with NO bold caption at all. Both passes above key off a
    # "Figure N." paragraph, so a document that captions its figures in the
    # surrounding prose - which this core's do, because the figures carry their
    # own titles and notes - left every image bare. A bare <img> matches none of
    # the figure CSS, so it renders at its intrinsic pixel width and runs off
    # the right edge of the page, silently. The alt text becomes the caption.
    def fig3(m):
        img = m.group(1)
        cls = "wide" if is_wide(img) else "inline"
        # No figcaption. Every figure in these documents carries its own title
        # inside the SVG, so alt text under the image would only repeat it.
        return f'<figure class="{cls}">{img}</figure>'

    html_text = re.sub(r"<p>(<img\b[^>]*/?>)</p>", fig3, html_text, flags=re.S)
    return html_text
...
@page {
    ...
    @top-left {
        content: "Avalon-MM SD Card Controller IP Core — """ + subtitle + """";
    ...
    @bottom-left {
        content: "altera_avalon_mm_sdcard_controller";
...
<div class="title-page">
  ...
  <div class="title-meta"><table>
    <tr><td>IP core</td><td><code>altera_avalon_mm_sdcard_controller</code></td></tr>
  ...
  <div class="title-foot">
    SD / SDHC / SDXC card controller for Avalon-MM, SPI mode.

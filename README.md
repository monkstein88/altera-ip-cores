...
| [`altera_avalon_mm_sdram_controller`](altera_avalon_mm_sdram_controller/README.md) | **Avalon-MM SDRAM Controller (per-bank rows)** · v1.0 · *Memory Interfaces and Controllers / Custom* | SDR SDRAM controller that keeps one open row *per bank* and treats a read/write turnaround as the datasheet does, rather than as a full row cycle. Drop-in replacement for the vendor core below | **Synthesised and closing 100 MHz on MAX 10 — not yet on a board.** 3.6–8.9× the vendor core on mixed and scattered traffic, for 3.4× the logic. 1992 testbench assertions across 12 configurations, 58-check board demonstration, 203 documentation claims checked |
...
| `altera_avalon_mm_sdram_controller` | yes — 12 configurations, plus Quartus Analysis & Synthesis | yes — 12 configurations, coverage, assertion non-vacuity | — |

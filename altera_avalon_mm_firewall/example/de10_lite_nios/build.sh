#!/usr/bin/env bash
# =============================================================================
# build.sh - build the Nios II Avalon-MM Firewall example end to end.
#
...
CORE="$HERE/../.."                      # the altera_avalon_mm_firewall component
COMMON="$HERE/../common"
...
step_qsys() {
    ...
    "$QSYS/qsys-generate" firewall_sys.qsys --synthesis=VERILOG \
...
step_sw() {
    ...
    cd "$HERE/software"
    rm -rf bsp
    # The driver is NOT copied in here. It comes from the component's
    # avl_mm_firewall_sw.tcl, which the BSP generator finds on the search path
    # and matches to the hardware by hw_class_name. That is the whole point of
    # shipping a _sw.tcl: adding the component to the system is enough.
    "$N2/nios2-bsp" hal bsp "$HERE/qsys/firewall_sys.sopcinfo" \
    ...
    echo "=== application ==="
    cd "$HERE/software"
    rm -rf obj Makefile firewall_app.elf
    "$N2/nios2-app-generate-makefile" --bsp-dir bsp --elf-name firewall_app.elf \
...
step_fpga() {
    echo "=== Quartus ==="
    cd "$HERE/quartus"
    "$QUARTUS_ROOT/quartus/bin/quartus_sh" --flow compile de10_lite_nios
}

step_clean() {
    rm -rf "$HERE/qsys/firewall_sys" "$HERE/qsys/.qsys_edit" \
    ...
    echo "cleaned (firewall_sys.qsys kept - it is tracked)"
}

...
int main(void)
{
    ...
    test_refresh();
    IOWR_ALTERA_AVALON_PIO_DATA(PIO_LED_BASE, 0x1FF);
    test_full_march();
    ...
    if (checks_failed == 0) {
        ...
        IOWR_ALTERA_AVALON_PIO_DATA(PIO_LED_BASE, 0x3FF);   /* all lit */
    } else {
        printf("  *** THERE ARE FAILURES ***\n");
        IOWR_ALTERA_AVALON_PIO_DATA(PIO_LED_BASE, 0x201);   /* ends lit */
    }
    printf("=============================================================\n");

...
#   ./build.sh clean    remove build artifacts (leaves sdram_perbank_sys.qsys)
#
...
step_qsys() {
    ...
    "$QSYS/qsys-generate" sdram_perbank_sys.qsys --synthesis=VERILOG \
        --search-path="$SEARCH" --output-directory=sdram_perbank_sys
}
...
step_fpga() {
    echo "=== Quartus ==="
    cd "$HERE/quartus"
    "$QUARTUS_ROOT/quartus/bin/quartus_sh" --flow compile de10_lite_sdram_demo
}

step_clean() {
    rm -rf "$HERE/qsys/sdram_perbank_sys" "$HERE/qsys/.qsys_edit" \
           "$HERE/simulation/verilator/.build" \
           "$HERE/quartus/db" "$HERE/quartus/incremental_db" \
           "$HERE/quartus/output_files"
    echo "cleaned (sdram_perbank_sys.qsys kept - it is tracked)"
}

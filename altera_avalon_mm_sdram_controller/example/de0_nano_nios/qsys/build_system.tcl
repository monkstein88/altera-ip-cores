...
create_system sdram_nios_nano_sys
...
# Smaller caches than the DE10-Lite's. The EP4CE22 has 608,256 memory bits
# against the 10M50's 1,677,312 - barely a third - and the on-chip RAM below
# already claims most of them.
set_instance_parameter_value cpu {icache_size} {2048}
set_instance_parameter_value cpu {dcache_size} {1024}
...
#
# 32 KB, a quarter of the DE10-Lite's, and the binding constraint is the
# NUMBER of M9K blocks rather than the total bits. The EP4CE22 has 66 of them
# at 8,192 bits each; a 32-bit-wide RAM uses one block per 256 words, so 64 KB
# claims 64 blocks and leaves nothing for the caches or the JTAG UART FIFOs -
# Quartus reports "Can't place all RAM cells" at 93% of the bits. 32 KB uses
# half of them and fits with room to spare.
#
# The program is 16 KB, built against the small C library - see build.sh -
# which leaves about 14 KB for stack and heap. It needs very little: the test
# allocates nothing and its buffers are the SDRAM itself.
#
# It would be simpler to run from the SDRAM, and wrong: a memory test
# executing out of the memory it is testing cannot report a failure it has
# just caused.
# ---------------------------------------------------------------------------
add_instance onchip_ram altera_avalon_onchip_memory2
set_instance_parameter_value onchip_ram {memorySize} {32768}
...
set_instance_parameter_value sysid {id} {3735928560}
...
apply_preset sdram "ISSI IS42S16160B-7 - DE0-Nano 32 MByte"
...
save_system sdram_nios_nano_sys.qsys
puts "=== system saved ==="

#!/usr/bin/env bash
# =============================================================================
# build.sh - build the Nios II SDRAM controller example end to end (DE0-Nano).
#
...
#   ./build.sh clean      remove build artifacts (leaves sdram_nios_nano_sys.qsys)
#
...
step_qsys() {
    ...
    "$QSYS/qsys-generate" sdram_nios_nano_sys.qsys --synthesis=VERILOG \
        --search-path="$SEARCH" --output-directory=sdram_nios_nano_sys
}

step_sw() {
    # SMALL C LIBRARY, unlike the DE10-Lite build. The full newlib printf is
    # most of a 75 KB image, and only 64 KB of on-chip RAM fits on this device.
    # The reduced printf handles %d %u %x %s %c and not floats, which is all
    # this program uses.
    echo "=== BSP ==="
    cd "$HERE/software"
    rm -rf bsp
    ...
    "$N2/nios2-bsp" hal bsp "$HERE/qsys/sdram_nios_nano_sys.sopcinfo" \
        --script "$SOPC_KIT_NIOS2/sdk2/bin/bsp-set-defaults.tcl" \
        --cmd set_setting hal.enable_reduced_device_drivers true \
        --cmd set_setting hal.enable_small_c_library true \
...
step_fpga() {
    echo "=== Quartus ==="
    cd "$HERE/quartus"
    "$QUARTUS_ROOT/quartus/bin/quartus_sh" --flow compile de0_nano_sdram_nios
}

step_clean() {
    rm -rf "$HERE/qsys/sdram_nios_nano_sys" "$HERE/qsys/.qsys_edit" \
    ...
    echo "cleaned (sdram_nios_nano_sys.qsys kept - it is tracked)"
}

...
#   ./run_bench.sh <module> <file> <part>   ... on another part
#
# <part> selects the device: 0 (default) is the DE10-Lite's IS42S16320D-7,
# 1 is the DE0-Nano's IS42S16160B-7. Only the custom core can be moved off the
# default part - Intel's is generated by gen_dut.sh with its geometry fixed,
# so asking for another part with that DUT is refused rather than silently
...
if [[ "$PART" != "0" && "$DUT_MODULE" == "sdram_mem_ctrl" ]]; then
    echo "error: Intel's core is generated for the DE10-Lite part only." >&2
    echo "       Regenerate it with a different geometry, or measure part 0." >&2
    exit 1
fi
...
# The part's geometry and timings reach the controller by defparam, which only
# makes sense for this project's core - see sdram_bench_tb.sv.
[[ "$DUT_MODULE" != "sdram_mem_ctrl" ]] && VFLAGS+=(-DDUT_PART)
